...
| End-User Message
| Short text input appended to the ticket post after marking as paid.
...
* Actions: **Tip by QR**, **Mark as Paid**.
* Status banners; QR code display.
...
* `private_comments`: boolean - When true, tip confirmations are posted as internal notes; when false, posted as public comments

**Hardcoded Configuration:**

* Tip presets: `100, 1000, 10000` sats (defined in `src/App.jsx`)
* Agent Lightning Address field: `user.custom_fields.lightning_address` (hardcoded in `src/services/zendeskService.js`)
* Payment mode: QR/LNURL-pay only (always enabled)
