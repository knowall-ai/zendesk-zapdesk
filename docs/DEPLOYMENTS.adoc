...
* **Tip presets**: Enter comma-separated values in satoshis
  - Example: `100,1000,10000`
  - These will appear as quick-select options for end-users
  - Recommended: `100` (1¢), `1000` (10¢), `10000` ($1 at current rates)
...
* **Enable QR Mode**: Check this box to enable QR code generation for BOLT11/LNURL payments
  - Recommended: Enabled (checked)

* **Fallback address**: Enter a default Lightning address to use if agent has no address configured
  - Format: `user@provider.com`
  - Useful for testing or as a backup

* **Ticket post visibility**: Select comment visibility for tip confirmations
  - Options: `public` (visible to requester) or `internal` (agents only)
  - Recommended: `public` to acknowledge tips publicly

* **Branding**: (Optional) Customize the app display
  - **App title**: Custom title for the sidebar
...
* Solution 2: Check the "Ticket post visibility" setting
