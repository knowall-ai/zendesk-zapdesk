...
export default function App({ client }) {
  ...
  useEffect(() => {
    ...
    async function init() {
      try {
        ...
        setError(err.message || i18n.t("errors.failed_to_load"));
        setLoading(false);
      }
    }
  ...
  async function onSelectAmount(amount) {
    ...
    if (!isValidLightningAddress(lightningAddress)) {
      setError(i18n.t("errors.no_lightning_address"));
      return;
    }
  }

  async function markAsPaid() {
    if (!ticketId) return setError(i18n.t("errors.no_ticket_id"));
    if (!selectedAmount) return setError(i18n.t("errors.no_tip_amount"));
    ...
    try {
      ...
      setError(err.message || i18n.t("errors.failed_to_post"));
    }
  }
      ...
      <header className="zd-header">
        ...
        <div className="zd-agent">
          ...
          <div>
            ...
            <div className="zd-sub">{i18n.t("ui.agent_subtitle")}</div>
          </div>
        </div>
      </header>
      ...
      <div className="zd-body">
        <div className="zd-tip-buttons">
          {TIP_AMOUNTS.map((a) => (
            ...
            >
              {a.toLocaleString()} {i18n.t("ui.sats")}
            </button>
          ))}
        </div>
        ...
        {selectedAmount && (
          <div className="zd-message-input">
            <label htmlFor="tip-message" className="zd-label">
              {i18n.t("ui.message_label")}
            </label>
            <textarea
              id="tip-message"
              className="zd-textarea"
              placeholder={i18n.t("ui.message_placeholder")}
              value={message}
